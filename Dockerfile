...
COPY target/demo-0.0.1-SNAPSHOT.jar /home/admin/app/demo.jar
################################################################################
...
CMD [ "java","-jar", "/home/admin/app/demo.jar"]
...
# Copy the executable from the "build" stage.COPY --from=build --chown=appuser:appuser /bin/hello.sh /bin/
